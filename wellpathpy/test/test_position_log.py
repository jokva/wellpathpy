...
@given(deviation_survey())
def test_resample_onto_unchanged_md_unchanged_deviation(survey):
    md, inc, azi = survey
    ...
    dev2 = resampled.deviation()

    np.testing.assert_allclose(dev.md, dev2.md)
    np.testing.assert_allclose(dev.inc, dev2.inc)
    np.testing.assert_allclose(dev.azi, dev2.azi)
